...
export default function Page() {
  ...
  return (
    <div className="max-w-4xl mx-auto py-10 px-6 font-sans">
      <h1 className="text-3xl font-bold mb-6 text-gray-900">Analytics Agent Dashboard</h1>
      <div className="flex flex-col gap-4">
        <textarea 
           value={query} 
           onChange={e => setQuery(e.target.value)} 
           className="w-full border rounded-lg p-3 min-h-[100px]"
           placeholder="Ask a question about BigQuery or Google Analytics data..."
        />
        <button 
           onClick={handleRun} 
      ...
      {context && (
        <div className="mt-8 p-4 bg-gray-50 text-gray-600 rounded-lg text-sm border">
          <p className="font-semibold mb-2">Internal Actions:</p>
          <pre>{context}</pre>
        </div>
      )}
